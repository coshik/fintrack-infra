...
Infrastructure runs on two AWS EC2 instances (Ubuntu, `t3.medium`) forming a `kubeadm`-provisioned Kubernetes cluster: one control-plane node, one worker node, joined via Calico as the pod network CNI.
...
**Phase 3 (Docker/K8s-Based Resilient Deployment) is complete.** The OOMKilled `account-service-v2` pod was fixed with a realistic memory limit plus a liveness probe as a retry strategy; the MongoDB PVC stuck `Pending` was fixed by installing `local-path-provisioner` (kubeadm clusters ship with zero storage classes by default) and recreating the StatefulSet. Along the way, a real cascading incident hit — repeated EC2 restarts left both nodes' disks under pressure, causing hundreds of evicted/orphaned pods across two namespaces — diagnosed back to root cause and cleaned up. The ResourceQuota was also found undersized against Istio's sidecar overhead and corrected using real measured usage.
...
- 2x AWS EC2 instances (Ubuntu, `t3.medium` or larger, 20GB+ root volume — a default ~7GB volume is genuinely too small for K8s + Jenkins + Docker together, confirmed the hard way)
...
- `local-path-provisioner` (storage class — kubeadm ships with none by default)
